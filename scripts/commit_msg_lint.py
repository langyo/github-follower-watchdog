...
Ported from langyo/wowsp scripts/commit_msg_lint.py (itself a faithful port
of the celestia-devtools `commit_msg.py` CI linter) so this repository
carries its own authoritative implementation; the whitelist and rules below
are kept in sync with the upstream source.
...
GITMOJI_WHITELIST: frozenset[str] = frozenset([
    ...
    "\U0001f4a9",        # 💩 :poop:
    "\u23ea",            # ⏪ :rewind:
    "\U0001f500",        # 🔀 :twisted_rightwards_arrow:
