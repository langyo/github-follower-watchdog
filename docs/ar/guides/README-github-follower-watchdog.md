...
**ضبط الإيقاع (توفير حصة الـ CI).** الـ cron يطلق كل ساعة كالمعتاد، لكن كل مقبض يحدد كم سيعمل فعليًا هو متغير مستودع عادي — يُضبط مرة واحدة في **Settings → Secrets and variables → Actions → Variables** ‏(`vars.*`) دون تعديل الـ workflow:

| المتغير | الافتراضي | المعنى |
| --- | --- | --- |
| `WATCH_INTERVAL_HOURS` | `1` | الحد الأدنى للساعات بين الفحوص المجدولة. عند `6` تنتهي الساعات البينية في ثوانٍ دون لمس الـ API — لا سجلات ولا بناء ولا نشر. تشغيلات **Run workflow** اليدوية تفحص فورًا دائمًا. |
| `WATCH_ENRICH_CAP` | `40` | عدد الحسابات الملفَّة في كل تشغيل (بحد أقصى 200؛ حقل Run workflow له الأولوية). |
| `WATCH_ENRICH_STALE_DAYS` | `30` | عدد الأيام قبل تحديث وقائع المتابع. |
| `WATCH_USER` | مالك الـ fork | مراقبة أي حساب عام آخر بدلًا منك. |

مثلًا، `WATCH_INTERVAL_HOURS=6` يقلّص حركة الـ API المجدولة بنحو 83% بينما يبقى الاتجاه والخط الزمني والدرجات محدَّثة أربع مرات يوميًا.
